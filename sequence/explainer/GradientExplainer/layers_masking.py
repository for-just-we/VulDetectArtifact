...
class layers:
    ...
    """
    now support layer: {'bi-lstm', 'bi-gru', 'liner', 'masking'}
    """
    ...
    @staticmethod
    @abstractmethod
    def lrp(inputs, weights, R, mask, eps, bias_factor, **kwargs):
        """
    ...
    @staticmethod
    @abstractmethod
    def deeplift(inputs, inputs_ref, weights, R, mask, eps, bias_factor, **kwargs):
        """
...
class Masking(layers):
    ...
    def forward(inputs, weights, mask, **kwargs):
        """
        kwargs:
            mask_value: same as Masking layer's mask_value in Keras

        return:
            intermediate_results: dict: "mask": new mask matrix
        """
        mask = np.all(inputs==kwargs['mask_value'], axis=1)
        for i,flag in enumerate(mask):
            if flag == True:
                inputs[i] *= 0

        return inputs.copy(), dict(mask=mask)


    @staticmethod
    def lrp(inputs, weights, R, mask, eps, bias_factor, **kwargs):
        """
        masking value does not change relevance
        """
        return R, 0.0           # Todo, add relevance sum
...
class Liner(layers):
    ...
    def lrp(inputs, weights, R, mask, eps, bias_factor, **kwargs):
        ...
        d = inputs.shape[0]//2
        intermediate_results = Liner.forward(inputs, weights, mask, **kwargs)[1]
    ...
    def deeplift(inputs, inputs_ref, weights, R, mask, eps, bias_factor, **kwargs):
       ...
       """
        ...
        intermediate_results_ref = Liner.forward(inputs_ref, weights, mask, **kwargs)[1]
...
class Bi_GRU(layers):
    ...
    def forward(inputs, weights, mask, **kwargs):
        ...
        for t in range(T):
            if mask[t]:
                h_Left[t]   = h_Left[t-1]       # only pass hidden state
            else:
    ...
    @staticmethod
    def lrp(inputs, weights, R, mask, eps=0.001, bias_factor=0.0, **kwargs):
        """
        kwargs
            recurrent_activation : activation function for GRU,LSTM, default hard_sigmoid
        ...
        intermediate_results    = Bi_GRU.forward(inputs, weights, mask, **kwargs)[1]        # Todo: clear redundant calc,
        ...
        if 'return_sequences' in kwargs.keys() and kwargs['return_sequences'] == True:
            ...
            for t in reversed(range(T)):            # T-1 -> 0
                if mask[t]:
                    Rx[t]         = np.zeros(inputs.shape[1])     # mask vector's relevance = 0
                    Rh_Left[t-1]  += Rh_Left[t]                   # pass the total relevance to next layer
                else:
                ...
                if mask_rev[t]:
                    Rx_rev[t]     = np.zeros(inputs.shape[1])
                    Rh_Right[t-1] += Rh_Right[t]
                else:
    ...
    def deeplift(inputs, inputs_ref, weights, R, mask, eps, bias_factor, **kwargs):
        ...
        intermediate_results    = Bi_GRU.forward(inputs, weights, mask, **kwargs)[1]
        h                       = intermediate_results['hidden_states']
        h_Left, h_Right         = h[:,:h.shape[1]//2], h[:,h.shape[1]//2:]                  # hidden state
        gates_pre_Left          = intermediate_results['gates_pre_Left']                    # load inter calc results
        gates_pre_Right         = intermediate_results['gates_pre_Right']
        ...
        intermediate_results_ref    = Bi_GRU.forward(inputs_ref, weights, mask, **kwargs)[1]
        ...
        # combine the calc procedure rather than calc before every time step
        ...
        if 'return_sequences' in kwargs.keys() and kwargs['return_sequences'] == True:
            ...
            for t in reversed(range(T)):            # T-1 -> 0
                ...
                if mask[t]:
                    Rx[t]         = np.zeros(inputs.shape[1])
                    Rh_Left[t-1]  += Rh_Left[t]
                else:
                    Rh_Left[t-1]  += lrp_linear(gates_Left[t,idx_z]*h_Left[t-1], np.identity(d), np.zeros((d)), h_Left[t], Rh_Left[t], 2*d, eps, bias_factor)
                    Rg_Left[t]    = lrp_linear((1-gates_Left[t,idx_z])*gates_Left[t,idx_h], np.identity(d), np.zeros((d)), h_Left[t], Rh_Left[t], 2*d, eps, bias_factor)
                    Rx[t]         = lrp_linear(inputs[t], Wxh_Left[idx_h].T, bxh_Left[idx_h], gates_pre_Left[t, idx_h], Rg_Left[t], d + input_col, eps, bias_factor)
...
class Bi_LSTM(layers):
    ...
    def lrp(inputs, weights, R, mask, eps, bias_factor, **kwargs):
        ...
        intermediate_results    = Bi_LSTM.forward(inputs, weights, mask, **kwargs)[1]    # forward pass, attention, this is for api sanity & easy, but the efficient could be low
        h                       = intermediate_results['hidden_states']
        h_Left, h_Right         = h[:,:h.shape[1]//2], h[:,h.shape[1]//2:]                      # hidden state
        ...
        if 'return_sequences' in kwargs.keys() and kwargs['return_sequences'] == True:
            ...
            for t in reversed(range(T)):       # T-1 -> 0
                if mask[t]:
                    Rx[t]           = np.zeros(inputs.shape[1])
                    Rh_Left[t-1]   += Rh_Left[t]
                    Rc_Left[t-1]    = Rc_Left[t]    # only every h gets the relevance from up layer, so only Rh needs to add up
                else:
                    Rc_Left[t]     += Rh_Left[t]
                ...
                if mask_rev[t]:
                    ...
                    Rx_rev[t]       = lrp_linear(inputs_rev[t], Wxh_Right[idx_g].T, bxh_Right[idx_g], gates_pre_Right[t, idx_g], Rg_Right[t], d+input_col, eps, bias_factor)
    ...
    def deeplift(inputs, inputs_ref, weights, R, mask, eps, bias_factor, **kwargs):
        ...
        # forward pass to get intermediate results
        intermediate_results    = Bi_LSTM.forward(inputs, weights, mask, **kwargs)[1]
        h                       = intermediate_results['hidden_states']
        h_Left, h_Right         = h[:,:h.shape[1]//2], h[:,h.shape[1]//2:]                  # hidden state
        gates_pre_Left          = intermediate_results['gates_pre_Left']                    # load inter calc results
        gates_pre_Right         = intermediate_results['gates_pre_Right']
        ...
        # forward pass for reference inputs
        intermediate_results_ref    = Bi_LSTM.forward(inputs_ref, weights, mask, **kwargs)[1]
        h_ref                       = intermediate_results_ref['hidden_states']
        h_Left_ref, h_Right_ref     = h_ref[:,:h_ref.shape[1]//2], h_ref[:,h_ref.shape[1]//2:]                  # hidden state
        gates_pre_Left_ref          = intermediate_results_ref['gates_pre_Left']                    # load inter calc results
        gates_pre_Right_ref         = intermediate_results_ref['gates_pre_Right']
